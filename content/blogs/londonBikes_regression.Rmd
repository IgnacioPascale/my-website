...
In the following workshop I will analyse the data set `londonBikes` from the UK Government. I will try to build a model in which to quantify the relationship or hired bikes and some explanatory variables using regression analysis, and shortly after I will use that model to predict the number hirings for a Wednesday in London in Summer with more than 15 degrees Celcius.
...
We want to predict how many bikes will be hired on a Wednesday with an average temperature of more than 15 degrees in summer. Let's filter our dataset first and then build predictions.
