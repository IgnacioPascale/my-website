...
keywords: ""
slug: data_pipeline
title: "From raw data to valuable insights: Building a data pipeline"
---
...
In our code, the methods associated with `load.py` loop over these file names and extract them in the local server. Then, the methods in `extraction.py` will first rename the files (by slicing over the numbers before "*_WEEK") and store the datasets in dictionaries with "file type" ("app", "new", etc) as keys and data frames as values.
...
Once the transformation was done, I needed to make sure that the data was not affected. I developed a script to perform quality check of the data.
...
I did not include code in this article, but if you want to check it, you can see it in the [github repo](). Do make sure to have a look at the readme first as it establishes more technical details associated with the process. Albeit being a kind of old project for me, I am open to any form of constructive criticism and code review.
